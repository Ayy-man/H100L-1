-- ============================================
-- MIGRATION: Sunday Real Ice Practice Booking System
-- ============================================
-- This migration adds Sunday practice booking functionality to the existing database.
...
--
-- What this adds:
-- - 2 new tables for slots and bookings
-- - 1 view for enriched booking data
-- - 4 database functions for booking operations
-- - RLS policies for security
-- - Indexes for performance
-- - Initial slot generation

-- ============================================
-- STEP 1: Create sunday_practice_slots table
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_sunday_slots_available ON public.sunday_practice_slots(practice_date, available_spots) WHERE is_active = true AND available_spots > 0;

-- Add comment
COMMENT ON TABLE public.sunday_practice_slots IS 'Weekly Sunday ice practice time slots with capacity management';

-- ============================================
-- STEP 2: Create sunday_bookings table
-- ============================================
...
CREATE INDEX IF NOT EXISTS idx_sunday_bookings_parent_email ON public.sunday_bookings(parent_email);
CREATE INDEX IF NOT EXISTS idx_sunday_bookings_date_lookup ON public.sunday_bookings(slot_id, booking_status) WHERE booking_status = 'confirmed';

-- Add comment
COMMENT ON TABLE public.sunday_bookings IS 'Player bookings for Sunday ice practice sessions with attendance tracking';

-- ============================================
-- STEP 3: Create sunday_bookings_detail view
-- ============================================

DROP VIEW IF EXISTS public.sunday_bookings_detail;

CREATE VIEW public.sunday_bookings_detail AS
SELECT
  -- Booking info
  b.id AS booking_id,
  b.booking_status,
...
-- ============================================
-- STEP 7: Generate initial slots (4 weeks ahead)
-- ============================================
...
-- MIGRATION COMPLETE
-- ============================================
-- Your Sunday Practice booking system is now ready!
--
-- Next steps:
-- 1. Deploy your frontend code to Vercel
-- 2. Set CRON_SECRET environment variable in Vercel
-- 3. Test the booking flow in parent dashboard
-- 4. Test the admin roster view
--
