-- ============================================
-- Fix Sunday Practice Payment Status Check
-- ============================================
-- Updates get_next_sunday_slot function to accept new payment statuses:
-- - 'succeeded' (Stripe confirmed)
-- - 'verified' (Admin confirmed)
-- - 'paid' (Legacy status - backwards compatibility)
--
-- Run this migration to fix "Unable to check eligibility" errors
...
BEGIN
  ...
  WHERE practice_date = v_next_sunday
    AND is_active = true
    AND (
      -- Match player category to slot categories
      v_player_category >= min_category
      AND v_player_category <= max_category
    );
...
-- Add helpful comment
COMMENT ON FUNCTION public.get_next_sunday_slot IS 'Returns next Sunday slot availability for a player. Accepts payment statuses: succeeded, verified, paid';
